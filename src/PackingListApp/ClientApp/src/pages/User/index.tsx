...
import { Layout, Input, Alert, Row, Col, Select, Dropdown, Menu, Checkbox } from "antd";
...
import {
    ...
    OccupationItem
} from "src/stores/occupation-store";
...
> {
    ...
    @autobind
    private async onDeleteItem(item: UserItem, state: ItemState) {
        var result = await this.UserItemsStore.deleteAsync(
            `${item.id}`,
            state
        );
        await this.load(this.state.query);
