...
import { Form, Spin, Select, Input, Checkbox, Modal, Row, Col, Alert, InputNumber, Table, Dropdown, Menu } from 'antd';
...
import { OccupationItemsStore } from '../../stores/occupation-store';
...
export class UserItemFormBody extends React.Component<ClassFormBodyProps> {
    ...
    loadOccupation = async () => {
        try {
            ...
            console.log("Occupations not found", error);
        }
    };
                ...
                <Col hidden={!this.state.isAdmin} span={12}>
                    <FormItem label={"Admin Type"}>
                        {getFieldDecorator(nameof<NewUserItem>('adminType'), {
                            initialValue: item.adminType || AdminType.NONE,
                        })(
                            <Select placeholder="Admin Type">
                                {
                                    Object.keys(AdminType).filter(a => a.match(/^\D/)).map((adminType: any) => (
                                        <option key={adminType} value={AdminType[adminType]}>{adminType}</option>))
                                }
                            </Select>
                        )}
                    </FormItem>
        ...
        return (
            <Modal
                ...
                {this.UserItemsStore.state.result && !this.UserItemsStore.state.result.isSuccess &&
                    ...
                    description={formatMessage(this.UserItemsStore.state.result.messages)}
                    />
                }
